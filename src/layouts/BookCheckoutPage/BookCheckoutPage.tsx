...
import { BookCheckoutAndReviewBox } from "./BookCheckoutAndReviewBox";
...
export const BookCheckoutPage = () => {
  ...
  //Loans Count State : should be 5 or less 
  const [currentLoansCount, setCurrtentLoansCount] = useState(0);
  ...
  useEffect(()=>{
    const fetchUserCurrentLoanCount =async () => {
      if(authState && authState.isAuthenticated){
        ...
        setCurrtentLoansCount(currentLoansCountResponseJson);
      }
      setIsLoadingCurrentCount(false);
    }
    fetchUserCurrentLoanCount().catch((err:any)=>{
      setIsLoadingCurrentCount(false);
      setHttpError(err.message);
    })
    ...
    const fetchUserCheckedOutBook = async () => {
      if(authState && authState.isAuthenticated){
        ...
        if(!bookCheckedOut.ok){
          throw new Error('Some thing went wrong in fetch checked out')
        }
  ...
  if (isLoading||isLoadingReview || isLoadingCurrentCount || isLoadingBookCheckedOut || isLoadingUserReview) {
